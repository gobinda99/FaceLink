...
class Wall_Updates {
	...
	{
	    $query = mysql_query("SELECT M.msg_id, M.uid_fk, M.message, M.created, U.username FROM messages M, users U  WHERE M.uid_fk=U.username and M.uid_fk='$uid' order by M.msg_id desc ") or die(mysql_error());
	...
	{
	    $query = mysql_query("SELECT C.com_id, C.uid_fk, C.comment, C.created, U.username FROM comments C, users U WHERE C.uid_fk=U.username and C.msg_id_fk='$msg_id' order by C.com_id asc ") or die(mysql_error());
	   while($row=mysql_fetch_array($query))
	    $data[]=$row;
        if(!empty($data))
		{
	...
	{
	    $query = mysql_query("SELECT email FROM `users` WHERE username='$uid'") or die(mysql_error());
	...
	$update=htmlentities($update);
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
        ...
        if ($update!=$result['message']) {
            ...
            $newquery = mysql_query("SELECT M.msg_id, M.uid_fk, M.message, M.created, U.username FROM messages M, users U where M.uid_fk=U.username and M.uid_fk='$uid' order by M.msg_id desc limit 1 ");
	...
	$comment=htmlentities($comment);
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
		...
		if ($comment!=$result['comment']) {
            ...
            $newquery = mysql_query("SELECT C.com_id, C.uid_fk, C.comment, C.msg_id_fk, C.created, U.username FROM comments C, users U where C.uid_fk=U.username and C.uid_fk='$uid' and C.msg_id_fk='$msg_id' order by C.com_id desc limit 1 ");
            $result = mysql_fetch_array($newquery);
